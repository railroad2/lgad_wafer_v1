...
def draw_C_pad(area=2, gain_layer=3, nplus_layer=4, metal_layer=7, oxide_layer=8, ild_layer=6): # area in mm^2
    ...
    ell_ild   = D.add_ref(pg.ellipse(radii=(radius, radius), angle_resolution=1, layer=ild_layer))
    ell_metal = D.add_ref(pg.ellipse(radii=(radius, radius), angle_resolution=1, layer=metal_layer))
    ell_oxide = D.add_ref(pg.ellipse(radii=(radius, radius), angle_resolution=1, layer=oxide_layer))
    ell_gain.center  = (0, 0)
...
def DrawTest():
    ...
    r_X_gain_p.center = (separation*2, 0)
    r_X_nplus.center  = (separation*3, 0)
    r_X_pstop.center  = (separation*4, 0)
    ...
    r_I_gain10_p.center = (separation*2, 0)
    r_I_gain20_p.center = (separation*2, r_I_jte20.y)
    r_I_nplus10.center  = (separation*3, 0)
    r_I_nplus20.center  = (separation*3, r_I_jte20.y)
    r_I_pstop10.center  = (separation*4, 0)
    r_I_pstop20.center  = (separation*4, r_I_jte20.y)
    ...
    r_C_none.center  = (separation*2, 0)
    r_C_nplus.center = (separation*3, 0)
    r_C_pstop.center = (separation*4, 0)

    
    label1 = d_vdp.add_ref(pg.text(text='Van der Pauw', layer=layerset['METAL'], size=100))
    label1.rotate(90)
    label1.center = (separation*-0.7, 0)

    label2 = d_res.add_ref(pg.text(text='Resistivity', layer=layerset['METAL'], size=100))
    label2.rotate(90)
    label2.center = (separation*-0.7, (r_I_jte10.center[1] + r_I_jte20.center[1])*0.5)

    label3 = d_cap.add_ref(pg.text(text='MOSCAP', layer=layerset['METAL'], size=100))
    label3.rotate(90)
    label3.center = (separation*-0.7, 0)

    r_vdp = D.add_ref(d_vdp)
    r_res = D.add_ref(d_res) 
    r_cap = D.add_ref(d_cap)

    r_cap.ymax = r_vdp.ymin - 100
    r_res.xmin = r_vdp.xmax + 250
    r_res.y = (r_vdp.y + r_cap.y)/2

    r_circle = d_cap1.add_ref(draw_C_pad())
    r_circle.center = (0, 0)
    label4_1 = d_cap1.add_ref(pg.text(text='MOSCAP (FULL)', layer=layerset['METAL'], size=100))
    ...
    label4_2.rotate(90)
    label4_3.rotate(90)
    label4_1.center = (r_circle.xmin-separation*0.7, 0)
    label4_2.center = (r_circle.xmin-separation*0.4, 0)
    label4_3.ymin = label4_2.ymax+10
    label4_3.xmin = label4_2.x-50

    dcy = D.center[1]
    r_cap1 = D.add_ref(d_cap1)
    r_cap1.xmax = r_vdp.xmin - 250
    r_cap1.y = dcy

    outline = pg.rectangle(size=(9500, 1700), layer=layerset['AUX'])
    outline = pg.outline(outline, distance=50, layer=layerset['AUX'])
    outline.center = D.center
    D.add_ref(outline)
